"use strict";
const ScoreEngine={
  apply(cid,targetType,targetId,delta,note){const p=activePeriod(cid);if(!p){uiAlert('먼저 기간을 시작해주세요 (설정 > 기간 관리)');return;}const pool=targetType==='team'?currentTeamScores(cid):currentScores(cid);pool[targetId]=(pool[targetId]||0)+delta;currentHistory(cid).push({id:uid(),targetType,targetId,delta,ts:Date.now(),note:note||''});persistAndRender();return pool[targetId];},
...
Object.assign(ScoreEngine,{
  attachSelection(chip,cid,targetType,id){chip.setAttribute('role','button');chip.tabIndex=0;chip.setAttribute('aria-pressed',String(DB.selection[cid].targetType===targetType&&DB.selection[cid].targetId===id));const choose=()=>ScoreEngine.select(cid,targetType,id);chip.onclick=choose;chip.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();choose();}};},
  ...
  renderTargets(cid){
    ...
    if(sel.targetType==='team'&&DB.teamsEnabled[cid]){const scoreMap=new Map(RankingModule.computeTeams(cid).map(t=>[t.id,t.score]));(DB.teams[cid]||[]).forEach(t=>{const chip=document.createElement('div');chip.className='stu-chip'+(sel.targetId===t.id?' selected':'');chip.innerHTML=`<div>👥 ${t.name}</div><div class="s num">${scoreMap.get(t.id)??0}</div>`;ScoreEngine.attachSelection(chip,cid,'team',t.id);strip.appendChild(chip);});const add=document.createElement('div');add.className='stu-chip add';add.textContent='+';add.title='팀 추가';add.onclick=()=>uiPrompt('새 팀 이름을 입력하세요','',name=>{const v=(name||'').trim();if(v)TeamModule.addTeam(cid,v);});strip.appendChild(add);}
    else{const scores=currentScores(cid);(DB.students[cid]||[]).forEach(s=>{const chip=document.createElement('div');chip.className='stu-chip'+(sel.targetType==='student'&&sel.targetId===s.id?' selected':'');chip.innerHTML=`<div>${s.name}</div><div class="s num">${scores[s.id]||0}</div>`;ScoreEngine.attachSelection(chip,cid,'student',s.id);strip.appendChild(chip);});const add=document.createElement('div');add.className='stu-chip add';add.textContent='+';add.title='학생 추가';add.onclick=()=>uiPrompt('새 학생 이름을 입력하세요','',name=>{const v=(name||'').trim();if(v)RosterModule.addStudent(cid,v);});strip.appendChild(add);}
  },
  renderControls(cid){if(!cid){$('#btnCorrect').disabled=true;$('#btnWrong').disabled=true;return;}const sel=DB.selection[cid],has=sel.targetType==='team'?(DB.teams[cid]||[]).some(t=>t.id===sel.targetId):(DB.students[cid]||[]).some(s=>s.id===sel.targetId);$('#btnCorrect').disabled=!has;$('#btnWrong').disabled=!has;$$('#brushRow button[data-v]').forEach(b=>b.disabled=!has);const q=typeof QuizModule!=='undefined'?QuizModule.liveQuiz(cid):null;$('#pointStepper').style.display=q&&q.mode==='B'?'none':'flex';const input=$('#ptVal');if(document.activeElement!==input)input.value=sel.points;}
});

"use strict";
ScoreEngine.renderHistory=function(){
...
ScoreEngine.init=function(){
  ...
  $('#ptVal').onchange=e=>ScoreEngine.setPoints(e.target.value);$('#ptVal').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();ScoreEngine.setPoints(e.target.value);e.target.blur();}};$('#btnCorrect').onclick=()=>ScoreEngine.scoreAction(true);$('#btnWrong').onclick=()=>ScoreEngine.scoreAction(false);$('#btnHistory').onclick=()=>{ScoreEngine.renderHistory();openModal('#modalHistory');};
};
