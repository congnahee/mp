"use strict";
/* ============================================================
   TEAM MODULE — 팀 데이터, 팀 설정 화면, 학생 배정
============================================================ */
const TeamModule = {
  ...
  render(cid){
    ...
    if(!enabled){body.innerHTML='<div class="hint" style="margin-top:10px;">팀 기능을 켜면 팀 이름을 등록하고 학생을 배정할 수 있어요. 그래프·랭킹·사다리를 팀 기준으로도 볼 수 있게 돼요.</div>';return;}
    ...
    const teams=DB.teams[cid]||[],teamList=$('#teamList');
    if(!teams.length)teamList.innerHTML='<div class="hint">아직 등록된 팀이 없어요.</div>';
    teams.forEach(t=>{
      const row=document.createElement('div');row.className='row-item';
      row.innerHTML=`<span>${t.name} <span style="color:var(--text-dim2);">(${TeamModule.membersOf(cid,t.id).length}명)</span></span><span class="row-right">${colorSwatchHTML('team',t.id,t.color)}<span class="x" data-edit style="color:var(--blue);">수정</span><span class="x" data-del>삭제</span></span>`;
      row.querySelector('[data-edit]').onclick=()=>uiPrompt('팀 이름을 수정하세요',t.name,name=>{const v=(name||'').trim();if(v)TeamModule.renameTeam(cid,t.id,v);});
      row.querySelector('[data-del]').onclick=()=>uiConfirm(`'${t.name}' 팀을 삭제할까요? 소속 학생은 미배정 상태가 돼요.`,()=>TeamModule.removeTeam(cid,t.id));teamList.appendChild(row);
    });
    $('#addTeamBtn').onclick=()=>{const input=$('#newTeamName'),v=input.value.trim();if(v){TeamModule.addTeam(cid,v);input.value='';}};
    $('#applyTeamColorsBtn').onclick=()=>{persistAndRender();closeModal('#modalSettings');};
    const assign=$('#assignList'),students=DB.students[cid]||[];if(!students.length)assign.innerHTML='<div class="hint">등록된 학생이 없어요.</div>';
    students.forEach(s=>{
      const row=document.createElement('div');row.className='row-item';const current=TeamModule.teamOf(cid,s.id),currentName=teams.find(t=>t.id===current)?.name||'미배정';
      row.innerHTML=`<span>${s.name}</span><div class="mini-dropdown"><button type="button" class="mini-dropdown-btn">${currentName} ▾</button><div class="mini-dropdown-menu"><button type="button" data-tid="" class="${!current?'active':''}">미배정</button>${teams.map(t=>`<button type="button" data-tid="${t.id}" class="${t.id===current?'active':''}">${t.name}</button>`).join('')}</div></div>`;
      const menu=row.querySelector('.mini-dropdown-menu');row.querySelector('.mini-dropdown-btn').onclick=e=>{e.stopPropagation();$$('.mini-dropdown-menu.open').forEach(m=>{if(m!==menu)m.classList.remove('open');});menu.classList.toggle('open');};
      menu.onclick=e=>{const b=e.target.closest('button');if(b)TeamModule.assignStudent(cid,s.id,b.dataset.tid||null);};assign.appendChild(row);
    });
  },
  init(){document.addEventListener('click',e=>{if(!e.target.closest('.mini-dropdown'))$$('.mini-dropdown-menu.open').forEach(m=>m.classList.remove('open'));});}
};
