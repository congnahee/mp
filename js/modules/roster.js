...
"use strict";
RosterModule.importStudents=function(file,cid){
...
"use strict";
Object.assign(RosterModule,{
  render(cid){RosterModule.renderClassTabs();RosterModule.renderClassInfo(cid);RosterModule.renderSettings(cid);},
  renderClassInfo(cid){
    const chip=$('#classInfoChip');if(!chip)return;if(!cid){chip.style.display='none';return;}
  ...
  renderClassTabs(){
    ...
    DB.classes.forEach(c=>{const b=document.createElement('div');b.className='class-tab'+(c.id===DB.activeClassId?' active':'');b.textContent=c.name;b.onclick=()=>RosterModule.setActive(c.id);wrap.appendChild(b);});
    const add=document.createElement('div');add.className='class-tab add';add.textContent='+ 반';add.onclick=()=>{openModal('#modalSettings');switchStab('class');};wrap.appendChild(add);
  },
  renderSettings(cid){
    const classList=$('#classList');classList.innerHTML='';
    DB.classes.forEach(c=>{const row=document.createElement('div');row.className='row-item';row.innerHTML=`<span>${c.name} ${c.id===cid?'(선택됨)':''}</span><span class="row-right"><span class="x" data-edit style="color:var(--blue);">수정</span><span class="x" data-del>삭제</span></span>`;row.querySelector('[data-edit]').onclick=()=>uiPrompt('반 이름을 수정하세요',c.name,name=>{const v=(name||'').trim();if(v)RosterModule.renameClass(c.id,v);});row.querySelector('[data-del]').onclick=()=>uiConfirm(`'${c.name}' 반을 삭제할까요? 모든 데이터가 사라져요.`,()=>RosterModule.removeClass(c.id));classList.appendChild(row);});
    RosterModule.renderClassChips(cid);
    const studentList=$('#studentList');studentList.innerHTML='';
    if(cid){
      const ids=new Set((DB.students[cid]||[]).map(s=>s.id));Array.from(RosterModule.selectedStudentIds).forEach(id=>{if(!ids.has(id))RosterModule.selectedStudentIds.delete(id);});
      (DB.students[cid]||[]).forEach(s=>{const row=document.createElement('div');row.className='row-item';row.innerHTML=`<span style="display:flex;align-items:center;"><input type="checkbox" class="row-check" data-check ${RosterModule.selectedStudentIds.has(s.id)?'checked':''}>${s.name}</span><span class="row-right">${colorSwatchHTML('student',s.id,s.color)}<span class="x" data-edit style="color:var(--blue);">수정</span><span class="x" data-del>삭제</span></span>`;row.querySelector('[data-check]').onchange=e=>{e.target.checked?RosterModule.selectedStudentIds.add(s.id):RosterModule.selectedStudentIds.delete(s.id);RosterModule.renderBulkBar(cid);};row.querySelector('[data-edit]').onclick=()=>uiPrompt('학생 이름을 수정하세요',s.name,name=>{const v=(name||'').trim();if(v)RosterModule.renameStudent(cid,s.id,v);});row.querySelector('[data-del]').onclick=()=>uiConfirm(`'${s.name}' 학생을 삭제할까요?`,()=>RosterModule.removeStudent(cid,s.id));studentList.appendChild(row);});
    }
  ...
  renderClassChips(cid){
    ...
    if(!DB.classes.length)wrap.innerHTML='<div class="hint">먼저 "반 관리" 탭에서 반을 추가해주세요.</div>';
  },
  renderBulkBar(cid){
    ...
    const total=(DB.students[cid]||[]).length,n=RosterModule.selectedStudentIds.size;count.textContent=`${n}명 선택`;selectAll.checked=total>0&&n===total;del.disabled=n===0;del.style.opacity=n===0?'.4':'1';
    if(DB.teamsEnabled[cid]){teamDrop.style.display=n>0?'block':'none';$('#bulkTeamMenu').innerHTML='<button type="button" data-team-assign="">미배정</button>'+(DB.teams[cid]||[]).map(t=>`<button type="button" data-team-assign="${t.id}">${t.name}</button>`).join('');}else teamDrop.style.display='none';
  }
});

"use strict";
RosterModule.init=function(){
